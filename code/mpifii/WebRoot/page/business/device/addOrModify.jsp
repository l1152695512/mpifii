...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="#" onclick="ajaxContent('/business/device');">设备管理</a><span class="divider">/</span></li>
	</ul>
</div>
<div class="row-fluid">
	<div class="box span12">
		<div class="box-header well">
			<h2>
				<i class="icon-edit"></i>
				<c:choose>
					<c:when test="${empty deviceInfo.id}">添加设备</c:when>
					<c:otherwise>修改设备</c:otherwise>
				</c:choose>
			</h2>
		</div>
		<div class="box-content">
			<form class="form-horizontal" id="editForm" action="${cxt}/business/device/save" method="POST">
				<input type="hidden" name="device.id" value="${deviceInfo.id}">
				<div class="control-group">
					<label class="control-label">SN</label>
					<div class="controls">
						<c:choose>
							<c:when test="${empty deviceInfo.id}">
								<input type="text" name="device.router_sn"
									class="input-xlarge" autocomplete="off" maxlength="16" vMin='16'
									vType="letterNumber" placeholder='16位0-9或a-f'>
							</c:when>
							<c:otherwise>
								<span class="sn_style">${deviceInfo.router_sn}</span>
							</c:otherwise>
						</c:choose>
				...
				<div class="control-group">
					...
					<div class="controls">
						<select name="device.type" class="input-xlarge">
							<option value="1" <c:if test="${deviceInfo.type == '1'}">selected="selected"</c:if>>route</option>
							<option value="2" <c:if test="${deviceInfo.type == '2'}">selected="selected"</c:if>>AP</option>
						</select>
					</div>
				</div>
<!-- 				<div class="control-group"> -->
...
<!-- 				</div> -->
				...
				<div class="form-actions">
					...
					<button class="btn" type="button"  onclick="ajaxContent('/business/device');">取消</button>
				</div>
			</form>
		</div>
	</div>
...
<<<<<<< HEAD
		...
		if(timeoutObj.val() != ""){
			var timeoutVal = parseInt(timeoutObj.val());
			if(timeoutVal<5 || timeoutVal>1600){
				hiddenInputColor(timeoutObj);
				showInputColor(timeoutObj, "error");
				timeoutObj.next(".help-inline").text("5-1600！");
				return;
			}
		}
=======

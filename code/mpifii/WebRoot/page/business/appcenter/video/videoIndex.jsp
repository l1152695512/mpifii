...
<form id="splitPage" class="form-horizontal" action="${pageContext.request.contextPath}/business/app/video" method="POST">
	<div>
		<ul class="breadcrumb">
			<li><a href="javascript:void(0);" onclick="ajaxContent('/content');">主页</a><span class="divider">/</span></li>
			<li>
				<a href="javascript:void(0);" onclick="ajaxContent('/business/app');">应用管理</a><span class="divider">/</span>
			</li>
			<li>
				<a href="javascript:void(0);" onclick="ajaxContent('/business/app/video');">视频管理</a>
			</li>
		</ul>
	</div>
	<div class="row-fluid">
		<div class="box span12">
			<div class="box-header well" >
				...
				<div class="box-icon">
					...
					<a href="javascript:void(0);" class="btn btn-round" title="添加应用" onclick="ajaxContent('/business/app/video/add');"><i class="icon-plus-sign"></i></a>
				</div>
			</div>
			<div class="box-content">
				<div class="search-content"><!-- 这里必须取名为search-content，否则搜索框离开焦点的点击事件不会触发搜索框的隐藏 -->
					<fieldset><!-- 这下面搜索的字段要与数据库里表的字段对应 -->
					  	<div class="control-group">
							<label class="control-label" for="focusedInput">视频名称：</label>
							<div class="controls">
				...
				<table class="table table-striped table-bordered bootstrap-datatable ">
					<thead>
						<tr>
							<th onclick="orderbyFun('name')">视频名称</th>
							<th onclick="orderbyFun('className')">视频分类</th>
							<th onclick="orderbyFun('tel')">视频链接</th>
							<th onclick="orderbyFun('status')">状态</th>
							<th width="250">操作</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="video" items="${splitPage.page.list}" >
							<tr>
								<td>${video.get("name")}</td>
								<td class="center">${video.get("typename")}</td>
								<td class="center">${video.get("link")}</td>
								<td class="center">${video.get("status")==0?'未发布':'已发布'}</td>
								<td class="center" data-id='${video.get("id")}' data-status='${video.get('status')}'>
									...
									<a class="btn btn-default btn-sm"  href="javascript:void(0);"> <i class="icon-wrench icon-black"></i>
									 <c:if test="${video.get('status')==0}"><span>发布</span></c:if>
									  <c:if test="${video.get('status')==1}"><span>取消发布</span></c:if>
									</a>
								</td>
							</tr>
						</c:forEach> 
...
<script type="text/javascript">
	...
	$("#splitPage .box-content tbody .icon-edit").parent().click(function(){
		...
		ajaxContent('/business/app/video/edit',{id:id});
	});
	$("#splitPage .box-content tbody .icon-trash").parent().click(function(){
		var id = $(this).parent().data("id");
		var url='/business/app/video/delete?id='+id;
	...
	$("#splitPage .box-content tbody .icon-wrench").parent().click(function(){
		...
		if(status==0){//未发布(发布)
			var url= "/business/app/video/changeStatus?id="+id;
			$.ajax({
				type : 'POST',
				dataType : "json",
				url :encodeURI(encodeURI(cxt + url)),

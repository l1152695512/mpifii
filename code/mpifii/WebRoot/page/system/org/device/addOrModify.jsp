...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="#" onclick="ajaxContent('/system/device');">设备管理</a><span class="divider">/</span></li>
		<c:choose>
			<c:when test="${empty device.id}"><li><a href="#">添加设备</a></li></c:when>
			<c:otherwise><li><a href="#">修改设备</a></li></c:otherwise>
		</c:choose>
	</ul>
</div>
<div class="row-fluid">
	<div class="box span12">
		<div class="box-header well">
			<h2>
				<i class="icon-edit"></i>
				<c:choose>
					<c:when test="${empty device.id}">添加设备</c:when>
					<c:otherwise>修改设备</c:otherwise>
				</c:choose>
			</h2>
		</div>
		<div class="box-content">
			<form class="form-horizontal" id="editForm" action="${cxt}/business/org/device/save" method="POST">
				<input type="hidden" name="device.id" value="${device.id}">
				<input type="hidden" name="device.shop_id" value="${device.shop_id}">
				<div class="control-group">
					<label class="control-label">SN</label>
					<div class="controls">
						<c:choose>
							<c:when test="${empty device.id}">
							...
							<c:otherwise>
								<span class="sn_style">${device.router_sn}</span>
							</c:otherwise>
						</c:choose>
				...
				<div class="control-group">
					...
					<div class="controls">
						<select name="device.type" class="input-xlarge">
							<option value="1" <c:if test="${device.type == '1'}">selected="selected"</c:if>>route</option>
							<option value="2" <c:if test="${device.type == '2'}">selected="selected"</c:if>>AP</option>
						</select>
					</div>
				</div>
...
// 	$("#editForm input[name='device.sn']").focusout(function(){
// 		var sn = $(this).val();

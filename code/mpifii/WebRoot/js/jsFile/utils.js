...
function setInputError($Obj,msg){
	hiddenInputColor($Obj);
	showInputColor($Obj, "error");
	$Obj.parent().children(".help-inline").text(msg);
}
...
//		console.debug(inputNode[0].tagName);//获取元素类型
...
//				if(nodeType == "input"){
					inputNode.parent().children(".help-inline").text(message);
//				}
				return false;
			}else if(vType == "length"){//如果只验证长度
				hiddenInputColor(inputNode);
				showInputColor(inputNode, "success");
				inputNode.parent().children(".help-inline").text("");
				return true;
			...
			if(value.length != 0){
				...
				}else if(vType == "numberZX"){//正整数(可包含小数)
					resultArr = utils_valiFunc.numberZX(value);
...
var utils_valiFunc = {
	 ...
	 * @returns
	 */
	"numberZX" : function(str){
		var result = str.match(/^[0-9]+[0]*(\.\d+)?$/);
	    if(result == null){
	    	return {"result" : false, "message" : "只能为正数！"};
	    }else{
	    	return {"result" : true, "message" : ""};
	    }
	},
	 ...
	 */
	"phone" : function(str){
		var partten = /^0?(13[0-9]|15[012356789]|18[0-9]|14[57])[0-9]{8}$/; ///^1[3,5]\d{9}$/;
		if(partten.test(str)){
			return {"result" : true, "message" : ""};
		}else{
			return {"result" : false, "message" : "格式不正确！"};
	 ...
	 */
	"url" : function(str){
		var partten = /^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i;
	    if(partten.test(str)){
	    	return {"result" : true, "message" : ""};
	    }else{
	    	return {"result" : false, "message" : "格式不正确！"};
